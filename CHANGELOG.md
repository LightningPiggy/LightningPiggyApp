0.7.5
=====
- New "Payment Sound" option in Settings -> Customise: the piggy can play a sound effect through the speaker when a payment is received (alongside the confetti). Choose Off (default), Pig Oink, Double Oink, Pig Squeal, or Random; tapping an option plays it immediately so you can compare them before leaving the screen. Boards without a speaker stay silent. Sounds are open-licensed farm recordings (oinks by Secretlondon, CC BY-SA 3.0; squeal by Foleyhaven, CC0), credited in res/sounds/CREDITS.md
...
- Fix the balance flashing "21.47 BTC" mid-update on large on-chain balances (e.g. 107 BTC briefly showing 21 then snapping back): the count-up animation uses LVGL's 32-bit animator, and any balance above 2³¹−1 sats (~21.47 BTC) overflowed to the clamped 2147483647. The animator is now skipped for balances that don't fit in int32 (and when nothing actually changed) — the balance is still painted at full precision directly. This also stops the broken animation from churning the event loop and making the BOOT switch button sluggish
- Fix the balance momentarily flashing a stale value when opening/closing Settings: if a wallet was running but its config key hadn't been stamped yet (`_active_wallet_key` left None after certain reconnect paths), every Settings round-trip saw a false "config changed" and needlessly restarted the wallet, repainting the last cached balance before the fresh fetch. `went_online` now stamps the key in its already-running early-return, so the wallet isn't restarted unless the config really changed
- Fix the ESP32 BOOT button (hardware wallet-switcher) flipping the active wallet and immediately flipping it back ("bounces back and doesn't switch"): mechanical release bounce was being detected as a second press. The watcher now waits for a debounced release before handling the press, plus a short cooldown between accepted presses, so one press = one switch
